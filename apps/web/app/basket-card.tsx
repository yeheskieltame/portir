...
export function BasketCard({ basket, icons, href = `/basket/${basket.slug}`, className = "", aspect = "aspect-[16/10]" }: { basket: Basket; icons?: Record<string, string | null>; href?: string; className?: string; aspect?: string }) {
  return (
    <Link href={href} className={`group relative block overflow-hidden rounded-3xl border border-line bg-[#0b111c] active:scale-[0.98] ${className}`}>
      <img src={basketImage(basket.slug)} alt="" className={`${aspect} w-full object-cover transition-transform duration-500 group-hover:scale-105`} />
      ...
      <span className="absolute inset-x-4 bottom-4">
        <span className="block text-[17px] leading-tight tracking-tight lg:text-xl">{basket.name}</span>
        {icons ? (
