...
import type { StockStats } from "@portir/core/binance";
...
const sample = (s: Omit<Stock, "icon" | "change24hPct"> & { change24hPct?: number }): Stock => ({ icon: null, change24hPct: null, stats: NO_STATS, ...s });
...
export const SAMPLE_STOCKS: Stock[] = [
  ...
  sample({ ticker: "QQQ", name: "Nasdaq 100 ETF", session: "closed", onchain: 601.3, reference: 600.2, change24hPct: 0.3 }),
  sample({ ticker: "SPY", name: "S&P 500 ETF", session: "closed", halted: "cash_dividend", onchain: 668.1, reference: 668.0, change24hPct: 0.05 }),
];
